...
describe("GetEventById.ts", () => {
  ...
  it("should handle error while getting events by id", async () => {
    ...
    (mockRepository.getEventById as jest.Mock).mockRejectedValueOnce(
      getError
    );

    await expect(
      getEventById.execute(expectedId)
    ).rejects.toThrow(GetEventsError);

    expect(mockRepository.getEventById).toHaveBeenCalledWith(
      expectedId
    );
  });
});
