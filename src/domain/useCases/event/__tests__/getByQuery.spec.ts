...
describe('GetEventByQuery.ts', () => {
  ...
  const mockRepository: Pick<EventRepository, 'getEventByQuery'> = {
    getEventByQuery: async (dayOfWeek?: string, desc?: string) => {
      if (desc) {
        return mockEvents.filter((event) =>
          event.description.toLowerCase().includes(desc.toLowerCase()),
        );
      } else if (dayOfWeek) {
        return mockEvents.filter(
          (event) => event.dayOfWeek.toLowerCase() === dayOfWeek.toLowerCase(),
        );
      } else {
        return mockEvents;
      }
  ...
  const getByQuery = new GetEventByQueryUseCase(
